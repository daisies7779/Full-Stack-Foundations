...
class webServerHandler(BaseHTTPRequestHandler):
        ...
        try:
            ...
            if self.path.endswith('/delete'):
                ...
                myRestaurantQuery = session.query(Restaurant).filter_by(id=
                    restaurantIDPath).one()
                if myRestaurantQuery:
                # alternatively code could be expressed like this
                # if myRestaurantQuery != [] :
                    self.send_response(200)
                    self.send_header('Content-type', 'text/html')
                    self.end_headers()
    ...
    def do_POST(self):
        try:
            if self.path.endswith("/delete"):
                #ctype, pdict is not necessary to implement delete
                ctype, pdict = cgi.parse_header(
                    self.headers.getheader('content-type'))
                restaurantIDPath = self.path.split("/")[2]
                myRestaurantQuery = session.query(Restaurant).filter_by(id = restaurantIDPath).one()
                #Reset the name field to the entry created in the form
                if myRestaurantQuery != []:
            ...
            if self.path.endswith("/edit"):
                ...
                if ctype == 'multipart/form-data':
                    ...
                    # Redirect bact to restaurants page
                    self.send_response(301)
                    self.send_header('Content-type', 'text/html')
                    self.send_header('Location', '/restaurants') 
                    self.end_headers()      
            ...
            if self.path.endswith("/restaurants/new"):
                ...
                if ctype == 'multipart/form-data':
                    ...
                    self.send_response(301)
                    self.send_header('Content-type', 'text/html')
                    # Redirects to original restaurants homepage
                    self.send_header('Location', '/restaurants') 
                    self.end_headers()
